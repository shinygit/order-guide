...
const RECEIVER_LOGIN = gql`
  mutation loginReceiver($login: String!, $password: String!) {
    loginReceiver(login: $login, password: $password) {
      ... on LoginReceiverError {
        loginError
        passwordError
      }
      ... on Token {
  ...
  return (
    <div className='flex justify-center flex-col items-center'>
      <div className='flex justify-between items-center py-3 mb-5 bg-blue-800 w-full'>
        <div className='flex flex-row items-center'>
          ...
          <span className='ml-1 text-xl text-gray-100 font-semibold'>
            Build-To Order Guides Receiver Login
          </span>
        </div>
      </div>
      <div className='w-full max-w-xs'>
        ...
        >
          <p className='text-center mb-1'>Please log in as receiver.</p>
          <div className='mb-4'>
            <label
            ...
            <input
              className={`${formStyle} ${
                loginError.loginError ? formErrorStyle : ''
              }`}
              onChange={({ target }) => setLoginField(target.value)}
              value={loginField}
              id='login'
              type='text'
            />
          ...
          <p className='text-red-500 text-s italic'>
            {loginError.loginError || loginError.passwordError}
          </p>
          <div>
            <button
